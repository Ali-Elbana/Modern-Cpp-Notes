...
When initializing built-in types (like `int`) with list initialization, the compiler prevents data loss. Here's an example:
...
- **Declaration:** Makes a name (variable) known but doesn't allocate memory.
...
- Using an initializer with `extern` overrides `extern` and creates a definition:
...
- Identifiers cannot contain two consecutive underscores (`__`) or start with an underscore followed by an uppercase letter (`_UpperCase`).
- Identifiers defined outside functions cannot start with an underscore (`_`).
...
```cpp
int reused = 42; // Global reused

int main() {
  int reused = 0; // Local reused hides global one

  // Output 1: Uses global reused (42)
  // Output 2: Uses local reused (0)
